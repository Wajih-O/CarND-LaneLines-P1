...
def horizon_crop(image: np.ndarray, horizon:Optional[int]=None) -> np.ndarray:
    ...
    if horizon is None:
        ...
        output_image[:image.shape[0]//2, :] = 0 # to keep it compatible with color image
    return output_image
...
def interest_region_crop(image):
    ...
    interest_region_polygon = np.array([list(map(lambda xy: (xy[0] , xy[1]), [left_bottom, apex, right_bottom]))])
...
def extract_lane(image, config=config) -> List[Segment]:
    ...
    segments = list(filter(lambda segment: not segment.nearly_vertical(.2) and not segment.nearly_horizontal(config.get("slope_threshold", 0.5)), [Segment(item) for item in np.squeeze(HoughLines()(edges_image))]))
    ...
    while len(segments) > 2: # iterative adaptation of the threshold
        cluster_dict, segments_store = agglomerate(segments, threshold=threshold, similarity=config.get("similarity"))
        # Filter/Sort and Select the 2 longest (merged/extended line) segments as the candidate for lane detection
        merged_segments = sorted(filter(lambda segment: segment is not None, [segments_store.get(key, None) for key in cluster_dict]), key=lambda segment: segment.length, reverse=True)
        segments =  merged_segments[:]
        threshold *= .95 #  more tolerance to the similarity threshold

    right, left = None, None
    left_right =  sorted(sorted(filter(lambda segment: segment.length > 40, segments), key=lambda segment: segment.length, reverse=True), key=lambda segment: segment.slope)[:2]
    if len(left_right) == 2:
        ...
        right = right.point_extend(right.intersection(Segment((0, height-1, width-1, height-1))), inclusive=False)
        left = left.point_extend(left.intersection(Segment((0, height-1, width-1, height-1))), inclusive=False)
