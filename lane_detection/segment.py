...
class Segment:
    ...
    def nearly_horizontal(self, threshold=0.2):
        if self.horizontal:
            return True
        return np.abs(self.slope) < threshold

    def nearly_vertical(self, threshold=0.1):
        return self.vertical or (
            np.abs(np.dot(self.vect, np.array((1, 0)))) / self.norm < threshold
        )
    ...
    def cross(self, other: "Segment") -> List[Tuple["Segment", "Segment"]]:
        ...
        return [
            (self, other),
            (
                Segment(self.ends[0].as_tuple + other.ends[0].as_tuple),
                Segment(self.ends[1].as_tuple + other.ends[1].as_tuple),
            ),
            (
                Segment(self.ends[0].as_tuple + other.ends[1].as_tuple),
                Segment(self.ends[0].as_tuple + other.ends[1].as_tuple),
            ),
        ]
    ...
    def min_config_cosine(self, other: "Segment"):
        return np.mean(
            [
                np.absolute(segment1.cosine_sim(segment2))
                for segment1, segment2 in self.cross(other)
            ]
        )
    ...
    def project(self, point: Point) -> Point:
        ...
        projected_x, projected_y = (
            (np.dot(self.vect, segment.vect) / ((self.norm) ** 2) * self.vect)
            + np.array([self.x1, self.y1])
        ).ravel()
        return Point(int(projected_x), int(projected_y))

    def point_extend(
        self, point: Point, distance_threshold=20, inclusive=True
    ) -> "Segment":
        ...
        candidates = [
            Segment((self.x1, self.y1, projected.x, projected.y)),
    ...
    def match_ends(self, other: "Segment"):
        ...
        matching_dict = {
            0: np.argmin(
                [
                    self.ends[0].euclidean_dist(other.ends[key])
                    for key in sorted(list(indices))
                ]
            )
        }
        indices.remove(
            matching_dict[0]
        )  # removes the matching of the first item (index == 0)
    ...
    def extend(self, other):
        ...
        candidates = [
            self.point_extend(Point(other.x1, other.y1)),
            self.point_extend(Point(other.x2, other.y2)),
        ]
        return candidates[np.argmax(list(map(lambda x: x.norm, candidates)))]

    def mutual_extend(
        self, other: "Segment", sort=False, reverse=True
    ) -> List["Segment"]:
    ...
    def weighted_merge(self, other) -> "Segment":
        ...
        extended = self.mutual_extend(
            other, sort=True, reverse=True
        )  # extended segments sorted by their norm
        matching = extended[0].match_ends(extended[1])

        match_1 = Segment(
            extended[0].ends[0].as_tuple + extended[1].ends[matching[0]].as_tuple
        )
        match_2 = Segment(
            extended[0].ends[1].as_tuple + extended[1].ends[matching[1]].as_tuple
        )
        ...
        end_0_translated = Segment(
            extended[0].ends[0].as_tuple
            + extended[0].ends[0].translate(Point(t1_x, t1_y)).as_tuple
        )
        end_1_translated = Segment(
            extended[0].ends[1].as_tuple
            + extended[0].ends[1].translate(Point(t2_x, t2_y)).as_tuple
        )

        return Segment(
            end_0_translated.ends[1].as_tuple + end_1_translated.ends[1].as_tuple
        )
